...
def get_file_path(filename):
    ...
    Se prueban (en orden):
    - la ruta tal como se pasó (absoluta o relativa al CWD)
    - relativa al directorio de este archivo
    - dentro de subcarpetas comunes ('imagenes', 'Imagenes', 'Imgs')
    - dentro de carpetas comunes en la raíz del repositorio ('Imagenes', 'Imgs', 'Musica', 'Sounds')
    - ruta con solo el basename en los anteriores lugares
    Devuelve la primera ruta existente o None.
    """
    # Si es ruta absoluta existente, devolverla
    if os.path.isabs(filename) and os.path.exists(filename):
        return filename

    # Prepara una lista de candidatos
    basename = os.path.basename(filename)
    candidates = []

    # Como fue pasado (relativo al cwd)
    ...
    candidates.append(os.path.join(SCRIPT_DIR, basename))
    candidates.append(os.path.join(SCRIPT_DIR, 'imagenes', basename))

    # En la raíz del repo
    candidates.append(os.path.join(REPO_ROOT, filename))
    candidates.append(os.path.join(REPO_ROOT, basename))
    candidates.append(os.path.join(REPO_ROOT, 'Imagenes', basename))
    candidates.append(os.path.join(REPO_ROOT, 'Imgs', basename))
    candidates.append(os.path.join(REPO_ROOT, 'Musica', basename))
    candidates.append(os.path.join(REPO_ROOT, 'Sounds', basename))

    # También probar posibles nombres antiguos/directorios del proyecto
    candidates.append(os.path.join(SCRIPT_DIR, 'delfin-version-frogge3r-main', basename))
    candidates.append(os.path.join(SCRIPT_DIR, 'delfin-version-frogger-main', basename))
