...
def cargar_imagen(nombre_archivo, ancho=TILE, alto=TILE, color_placeholder=(150,150,150)):
    rutas_posibles = [
        os.path.join("Imgs", nombre_archivo),
        nombre_archivo
    ]
...
def crear_basura_y_peligros():
    ...
    # Basura en posiciones aleatorias pero separadas
    posiciones_basura = []
    for _ in range(5):
        while True:
            x = random.randint(50, ANCHO-50)
            y = random.choice([TILE*1, TILE*2, TILE*3])
            # Verificar que no esté demasiado cerca de otra basura
    ...
    for y in [TILE*4, TILE*5]:
        for _ in range(2):  # Reducir cantidad para menos congestión
            x = random.randint(0, ANCHO)
            vel = random.choice([-2, 2])  # Velocidad más lenta
            peligro_group.add(Peligro(x, y, vel))

def draw_background():
    # Fondo estatico para eliminar flickering
    screen.blit(fondo, (0, 0))

def draw_hud():
    font = pygame.font.SysFont(None, 28)
    t1 = font.render(f"Vidas: {jugador.vidas}", True, BLANCO)
    t2 = font.render(f"Basura: {'SÍ' if jugador.tiene_basura else 'NO'}", True, BLANCO)
    t3 = font.render(f"Limpiezas: {sum(ocupadas)}/5", True, BLANCO)
    t4 = font.render(f"Puntaje: {puntaje}", True, BLANCO)
    ...
    while pausa_activa:
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                # Si cierran la ventana durante la pausa, salir del juego
                pygame.quit()
                sys.exit()
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_UP:
                    ...
                    # Permitir salir del menú pausa con ESC o P también
                    pausa_activa = False
...
while True:
    ...
    if not jugador.invulnerable and pygame.sprite.spritecollideany(jugador, peligro_group):
        ...
        reset_player()
        jugador.hacer_invulnerable(60)  # 1 segundo de invulnerabilidad
        if jugador.vidas <= 0:
